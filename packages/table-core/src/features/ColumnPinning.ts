...
export const ColumnPinning: TableFeature = {
  ...
  createRow: <TData extends RowData>(
    ...
    row.getLeftVisibleCells = () => {
      const [allCells, left] = [row._getAllVisibleCells(), table.getState().columnPinning.left];
      const cells = (left ?? [])
          .map(
    ...
    row.getRightVisibleCells = () => {
      const [allCells, right] = [row._getAllVisibleCells(), table.getState().columnPinning.right];
      const cells = (right ?? [])
        .map(
          (columnId) => allCells.find((cell) => cell.column.id === columnId)!,
        )
